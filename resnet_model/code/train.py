...
all_files = [os.path.join(dp, f) for dp, _, filenames in os.walk(train_dir)
             for f in filenames if f.lower().endswith(('.jpg', '.jpeg', '.png'))]

# wybieramy top4 klasy
labels_all = [os.path.basename(os.path.dirname(f)) for f in all_files]
counts = Counter(labels_all)
top4 = [cls for cls, _ in counts.most_common(4)]
all_files = [f for f in all_files if os.path.basename(os.path.dirname(f)) in top4]
class_names = sorted(top4)
print("Final class names:", class_names)

epochs = int(os.environ.get('EPOCHS', 10))
batch_size = int(os.environ.get('BATCH_SIZE', 32))

train_kfold(all_files, class_names, num_classes=len(class_names),
